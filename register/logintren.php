...
if( $user->is_logged_in() ){ header('Location: indextren1.php'); } // none exist page
...
if(isset($_POST['submit'])){
	...
	$tren=true;
	if($user->login($username,$password,$tren)){ 
		$_SESSION['username_s'] = $username;
		if (($_SESSION['username_s']=='centrekiev')) {$_SESSION['admin']=true; } else {$_SESSION['admin']=false;} 
		header('Location: memberpagetren.php');
		exit;
		
	} else {
		$error[] = "Невірне ім'я користувача/емейл/ чи пароль або ваш обліковий запис не активовано.";
	}
	    ...
	    <div class="col-xs-12 col-sm-8 col-md-6 col-sm-offset-2 col-md-offset-3">
			<form role="form" method="post" action="" autocomplete="off">
				...
				<div class="form-group">
					<input type="text" name="username" id="username" class="form-control input-lg" placeholder="Ім'я для входу/емейл/" value="<?php if(isset($error)){ echo $_POST['username']; } ?>" tabindex="1">
				</div>
				...
				<div class="row">
					...
					<p><a href='../trening/trening-2018.shtml'>Відміна</a></p>
				</div>
			</form>
		</div>
	</div>
