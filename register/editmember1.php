...
//----------------------------------------------
			...
			try {
				$stmt = $db->prepare('SELECT COUNT(*) as count FROM members WHERE groupcat = :groupcat GROUP BY groupcat ');
				$stmt->execute(array('groupcat' => $groupcat));
			...
			<form name="form_reg" role="form" method="post" action="" autocomplete="off">
				...
				<div class="row">
					...
					<label>Email</label>						
						<div class="form-group">
							...
							value="<?php if(isset($error)){ echo $_POST['email']; } else echo $email;?>" tabindex="2">
						</div>						
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-xs-6 col-sm-6 col-md-3">
						<div class="form-group">
							...
							value="<?php if(isset($error)){ echo $_POST['birthday']; } else echo $row['birthday'];?>" title="Дата народження" tabindex="8">
						</div>
					</div>		
				</div>
				...
				<div class="row container	">
					<div class="col-xs-3 col-sm-3 col-md-3" style="min-height: 30px;">
						<div class="radio" >
						  <label>
							<input type="radio" name="misceroboty"  value="1" <?php if(isset($error)) {if ($_POST['misceroboty']=="1") echo "checked"; } elseif ($row['misceroboty']=="1") {echo "checked";} ?> onclick="document.form_reg.centrevlada.disabled=false; document.form_reg.organvlady.disabled=true;  document.form_reg.organvlady.value='';" >
							центральний
						  </label>
						</div>
						<div class="radio">
						  <label>
							<input type="radio" name="misceroboty"  value="2" <?php if(isset($error)) {if ($_POST['misceroboty']=="2") echo "checked"; } elseif ($row['misceroboty']=="2") {echo "checked";} ?> onclick="document.form_reg.centrevlada.disabled=true;  document.form_reg.organvlady.disabled=false; document.form_reg.centrevlada.value='';" >
							інший
						  </label>						  
							...
							<select name="centrevlada" id="centrevlada" <?php if(isset($error) && ($_POST['misceroboty']=="2")){ echo "disabled"; } elseif ($row['misceroboty']=="2") {echo "disabled";} ?> class="form-control ">
								...
								<?php
									...
									for ($vc=1; $vc<=$countvlada; $vc++){
										echo '<option  ';
									if (isset($error) && !empty($_POST['centrevlada'])) { if ($_POST['centrevlada']==$vc)  echo ' selected '; } 
											elseif (!isset($error) &&($row['centrevlada']==$vc)) {echo ' selected ';}
										//echo '	value="'.$vc.'">'.vladaname($vc).'</option>';
										list ($vladaname, $vladamembers) = vlada_name_membr($vc);
										echo '	value="'.$vc.'">'.$vladaname.'('.$vladamembers.')</option>';
									}
								?>
							</select>
							
							<input type="text" <?php if(isset($error)) {if ($_POST['misceroboty']=="1") echo "disabled"; } elseif ($row['misceroboty']=="1") {echo "disabled";} ?> 
							name="organvlady" id="organvlady" class="form-control " placeholder="Інший орган влади" 
							value="<?php if(isset($error)) {if($_POST['misceroboty']=="2") echo $_POST['organvlady']; } elseif ($row['misceroboty']=="2") {echo $row['organvlady'];}?>" title="Орган влади" tabindex="9">						
				...
				<div class="row">
					<div class="col-xs-6 col-sm-6 col-md-3">
						<div class="form-group">
							...
							value="<?php if(isset($error)){ echo $_POST['adresamisto']; } else echo $row['adresamisto'];?>" title="Адреса місця роботи, нас. пункт" tabindex="10">
						</div>
					</div>
					<div class="col-xs-6 col-sm-6 col-md-6">
						<div class="form-group">
							<input type="text" name="adresaraion" id="adresaraion" class="form-control " placeholder="Район" 
							value="<?php  if(isset($error)){ echo $_POST['adresaraion']; } else echo $row['adresaraion'];?>" title="Адреса місця роботи, район" tabindex="11">
						</div>
					</div>
				</div>
				<div class="row">
					<div class="col-xs-6 col-sm-6 col-md-12">
						<div class="form-group">
							...
							value="<?php  if(isset($error)){ echo $_POST['adresaoblast']; } else echo $row['adresaoblast'];?>" title="Адреса місця роботи, область" tabindex="12">
						</div>
					</div>
				</div>
				...
				<div class="row">
					<div class="col-xs-6 col-sm-6 col-md-12">
						<div class="form-group">
							...
							value="<?php  if(isset($error)){ echo $_POST['posada']; } else echo $row['posada'];?>" title="Повна назва посади" tabindex="13">
						</div>
					</div>
				</div>
				...
				<div class="row">
					<div class="col-xs-3 col-sm-3 col-md-3">
						<div class="form-group">
							...
							value="<?php  if(isset($error)){ echo $_POST['slugtelefon']; } else echo $row['slugtelefon'];?>" title="Службовий телефон 0YYXXXXXXX" tabindex="14">
						</div>
					</div>
				</div>				
				...
				<div class="row">
					<div class="col-xs-3 col-sm-3 col-md-3">
						...
						<select size="1" name="category" id="category" onchange="document.form_reg.hidegroplata.value=''; document.form_reg.hiderang.value=''; selectGroupRang(); "  class="form-control">
							...
							<option value="1" <?php if(isset($error)) {if ($_POST['category']=='1'){ echo "selected"; }} elseif ($row['category']=='1'){ echo "selected"; } ?>  >А</option>
							<option value="2" <?php if((isset($error) && ($_POST['category']=='2')) || (!isset($error)&&($row['category']=='2'))){ echo "selected"; } ?>  >Б</option>
							<option value="3" <?php if((isset($error) && ($_POST['category']=='3')) || (!isset($error)&&($row['category']=='3'))){ echo "selected"; } ?>  >В</option>
						</select>
						</div>
					</div>				
				</div>				
				
				<input type="hidden" name="hidegroplata" id="hidegroplata" class="form-control "  value="<?php if(isset($error)){ echo $_POST['groplata']; } else echo $row['groplata'];?>" title="Вибрана група оплати" >
				<input type="hidden" name="hiderang" id="hiderang" class="form-control "  value="<?php if(isset($error)){ echo $_POST['rang']; } else echo $row['rang'];?>" title="Вибран ранг" >
				...
				<div class="row">
					<div class="col-xs-3 col-sm-3 col-md-3">
						<div class="form-group">
							...
							value="<?php if(isset($error)){ echo $_POST['stagdergyear']; } else echo $row['stagdergyear'];?>" title="Стаж державної служби, років" tabindex="17">
						</div>
					</div>
					<div class="col-xs-3 col-sm-3 col-md-3">
						<div class="form-group">
							<input type="number" min="0" max="12" name="stagdergmonth" id="stagdergmonth" class="form-control " placeholder="місяців" 
							value="<?php if(isset($error)){ echo $_POST['stagdergmonth']; } else echo $row['stagdergmonth'];?>" title="Стаж державної служби, місяців" tabindex="18">
						</div>
					</div>					
				</div>				
				...
				<div class="row">
					<div class="col-xs-3 col-sm-3 col-md-3">
						<div class="form-group">
							...
							value="<?php if(isset($error)){ echo $_POST['stagposadayear']; } else echo $row['stagposadayear'];?>" title="Стаж роботи на посаді, років" tabindex="19">
						</div>
					</div>
					<div class="col-xs-3 col-sm-3 col-md-3">
						<div class="form-group">
							<input type="number" min="0" max="12" name="stagposadamonth" id="stagposadamonth" class="form-control " placeholder="місяців" 
							value="<?php if(isset($error)){ echo $_POST['stagposadamonth']; } else echo $row['stagposadamonth'];?>" title="Стаж роботи на посаді, місяців" tabindex="20">
						</div>
					</div>					
				</div>	
				...
				<div class="row">
					<div class="col-xs-6 col-sm-6 col-md-12">
						<div class="form-group">
							...
							value="<?php if(isset($error)){ echo $_POST['tema']; } else echo $row['tema'];?>" title="Тема випускної роботи" tabindex="28">
						</div>
					</div>
				</div>
				...
				<div class="row">
					<div class="col-xs-6 col-sm-6 col-md-5" >
						<div class="form-group"  >
							<label class="radio-inline" ><input type="radio" name="pidvishenia" class="form-control " style="height: 14px;" value="1" 
							<?php if ((!isset($error)&&($row['pidvishenia']=='1')) || (isset($error)&& ($_POST['pidvishenia']=='1'))) echo "checked"; ?> tabindex="29">Так</label>
							<label class="radio-inline" ><input type="radio" name="pidvishenia" class="form-control " style="height: 14px;" value="2" 
							<?php if ((!isset($error)&&($row['pidvishenia']=='2')) || (isset($error)&&( $_POST['pidvishenia']=='2'))) echo "checked"; ?>  tabindex="30">&ensp;Ні&ensp;</label>
